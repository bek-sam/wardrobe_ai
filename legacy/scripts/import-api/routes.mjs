...
export function createRouter({
  ...
  return async function handler(req, res, next) {
    ...
    try {
      ...
      const wardrobeDeleteMatch = url.pathname.match(
        /^\/api\/import\/wardrobe\/(import-[a-f0-9-]{36})$/i,
      );
      if (wardrobeDeleteMatch && req.method === "DELETE") {
        ...
        if (next.length === records.length)
          return json(res, 404, { error: "Imported wardrobe item not found" });
      ...
      if (url.pathname === API_ROOT && req.method === "POST") {
        ...
        for (const metadata of detected) {
          ...
          const cropStage = {
            ...stageState(),
            status: "review",
            assetUrl: `${ASSET_ROOT}/${id}/${cropFile}`,
            updatedAt: now,
          };
          const job = {
            id,
      ...
      if (url.pathname === API_ROOT && req.method === "GET") {
        ...
        const hiddenJobs = loadedJobs.filter(
          (job) =>
            job.status === "complete" ||
        ...
        await Promise.all(
          hiddenJobs.map((job) => rm(path.join(jobsDir, job.id), { recursive: true, force: true })),
        );
        const jobs = loadedJobs
          .filter((job) => !hiddenJobs.includes(job))
      ...
      if (cleanupAction && req.method === "POST") {
        ...
        if (stage.status !== "failed" || !stage.failedAssetUrl) {
          throw Object.assign(new Error("No failed garment source is available for cleanup"), {
            status: 409,
          });
        }
        const input = await body(req);
        ...
        const sourceName = path.basename(
          new URL(stage.failedAssetUrl, "http://localhost").pathname,
        );
      ...
      const stageMatch = action.match(
        /^stages\/(crop|garment|modeled)\/(approve|reject|regenerate)$/,
      );
      if (stageMatch && req.method === "POST") {
        const [, stageName, decision] = stageMatch;
        if (!STAGES.has(stageName))
          throw Object.assign(new Error("Invalid stage"), { status: 400 });
        if (decision === "regenerate") {
          if (stageName === "crop")
            throw Object.assign(new Error("Upload the image again to create new crops"), {
              status: 400,
            });
          const input = await body(req);
          job.stages[stageName].prompt =
            typeof input.prompt === "string" ? input.prompt.trim().slice(0, 1200) || null : null;
        ...
        if (!DECISIONS.has(decision) || job.stages[stageName].status !== "review")
          throw Object.assign(new Error("Stage is not ready for review"), { status: 409 });
        ...
        const startGarment =
          stageName === "crop" && decision === "approve" && job.stages.garment.status === "pending";
        const startModeled =
          stageName === "garment" &&
          decision === "approve" &&
