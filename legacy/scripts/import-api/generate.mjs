...
import { ASSET_ROOT } from "./constants.mjs";
import { chooseChromaKey, removeChromaBackground } from "./chroma-key.mjs";
import { openAIEdit } from "./openai-client.mjs";
import { buildGarmentPrompt, DEFAULT_MODELED_PROMPT } from "./prompts.mjs";

export function createGenerator({ jobsDir, root, setting, apiBaseUrl, store, options = {} }) {
  ...
  async function generate(job, stageName) {
    ...
    const task = (async () => {
      const current = await store.loadJob(job.id);
      ...
      try {
        ...
        const sourceFile =
          stageName === "garment" && current.internal.cropFile
            ? current.internal.cropFile
            : current.internal.originalFile;
        const original = {
          data: await readFile(path.join(dir, sourceFile)),
          mime: "image/png",
          name: sourceFile,
        };
        let bytes;
        if (stageName === "garment") {
          ...
          const basePrompt =
            options.garmentPrompt || buildGarmentPrompt(current.metadata, chromaKeyUsed);
          bytes = await openAIEdit({
            ...
            size: "1024x1024",
            images: [original],
            prompt: current.stages.garment.prompt
          ...
          const garment = {
            data: await readFile(garmentFile),
            mime: "image/png",
            name: "garment.png",
          };
          const modelPath = path.resolve(
            root,
            setting("WARDROBE_MODEL_REFERENCE", "data/model-reference.png"),
          );
          let modelData;
          try {
            ...
            if (error.code === "ENOENT")
              throw new Error(
                `Model reference not found at ${modelPath}. Set WARDROBE_MODEL_REFERENCE or add data/model-reference.png.`,
              );
            throw error;
          }
          ...
          bytes = await openAIEdit({
            ...
            prompt: current.stages.modeled.prompt
              ? `${basePrompt}\nUser regeneration direction: ${current.stages.modeled.prompt}`
              : basePrompt,
          });
        }
        await writeFile(output, bytes);
        const fresh = await store.loadJob(current.id);
        ...
        fresh.stages[stageName].updatedAt = new Date().toISOString();
        if (typeof failedAssetUrl === "string")
          fresh.stages[stageName].failedAssetUrl = failedAssetUrl;
        if (chromaKeyUsed) fresh.stages[stageName].chromaKey = chromaKeyUsed;
        await store.saveJob(fresh);
      }
