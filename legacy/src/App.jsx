...
import { TYPE_MAP, TYPES, TYPE_ORDER } from "./constants/types.js";
...
import { WardrobeImportFlow } from "./features/import/WardrobeImportFlow.jsx";
...
export function App() {
  ...
  const visibleItems = useMemo(() => {
    const filtered =
      activeType === "all" ? items : items.filter((item) => item.part === activeType);
    ...
    if (id.startsWith("import-")) {
      try {
        ...
        if (!response.ok && response.status !== 404)
          throw new Error("Could not delete the imported item.");
      } catch (requestError) {
  ...
  const addImportedItem = useCallback((newItem) => {
    setItems((current) =>
      current.some((item) => item.id === newItem.id) ? current : [...current, newItem],
    );
  }, []);
  ...
  const attachImportedModeledImage = useCallback((jobId, modeledImage) => {
    ...
    setItems((current) =>
      current.map((item) => (item.id === id ? { ...item, modeledImage } : item)),
    );
  }, []);
    ...
    <div className={`app-shell${selectedItem ? " has-selection" : ""}`}>
      <main className="gallery-pane">
        ...
        {!error && !loading && !items.length && (
          <p className="status empty">Drop, paste, or add a photo to import your first piece.</p>
        )}
