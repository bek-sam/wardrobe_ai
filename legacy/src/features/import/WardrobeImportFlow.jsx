...
export function WardrobeImportFlow({ onGarmentApproved, onModeledApproved }) {
  ...
  useEffect(() => {
    api(CONFIG_API)
      .then(setSetup)
      .catch((requestError) => setSetup({ ready: false, error: requestError.message }));
    api(API)
      .then((storedJobs) => {
        ...
        try {
          ...
          if (!createdJobs.length && result.noClothingDetected) {
            setNotice({
              ...
              detail: `We couldn’t find a distinct wearable item in ${file.name}. Try a clearer or more tightly framed image.`,
            });
            setOpen(true);
            continue;
          }
          ...
          setDrafts((current) => ({
            ...current,
            ...Object.fromEntries(createdJobs.map((job) => [job.id, defaultDraft(job)])),
          }));
        } catch (requestError) {
          setError(requestError.message);
        }
    ...
    try {
      if (stage === "garment" && action === "approve") {
        ...
        const updated = await api(`${API}/${job.id}/stages/${stage}/${action}`, {
          method: "POST",
          body: action === "regenerate" ? JSON.stringify({ prompt }) : undefined,
        });
        const removeFromQueue =
          action === "reject" || (stage === "modeled" && action === "approve");
        ...
        if (removeFromQueue) {
          setDrafts((current) =>
            Object.fromEntries(Object.entries(current).filter(([id]) => id !== job.id)),
          );
        ...
        if (stage === "modeled" && action === "approve")
          onModeledApproved?.(job.id, `/api/import/library/import-${job.id}-modeled.png`);
      }
    } catch (requestError) {
      setError(requestError.message);
    } finally {
  ...
  const performCleanup = async (job, action, requestedTolerance) => {
    ...
    try {
      const tolerance =
        requestedTolerance ??
        cleanupTolerances[job.id] ??
        job.stages?.garment?.cleanupTolerance ??
        46;
      const updated = await api(`${API}/${job.id}/stages/garment/cleanup-${action}`, {
        method: "POST",
        body: JSON.stringify({ tolerance }),
      });
      setJobs((current) => current.map((item) => (item.id === job.id ? updated : item)));
      setCleanupTolerances((current) => ({
        ...current,
        [job.id]: updated.stages?.garment?.cleanupTolerance ?? tolerance,
      }));
  ...
  const selectedReviewJob = jobs.find(
    (job) => job.id === selectedReviewId && (reviewStageFor(job) || hasCleanupFailure(job)),
  );
  const reviewJob =
    selectedReviewJob ||
    jobs.find((job) => reviewStageFor(job)) ||
    jobs.find((job) => hasCleanupFailure(job)) ||
    active;
  ...
  return (
    <>
      ...
      />
      <div
        className="import-drop-overlay"
        data-active={dragging && !setupRequired}
        aria-hidden={!dragging || setupRequired}
      >
        <div className="import-drop-target is-over">
          <UploadSimple size={34} weight="light" />
          <h2>Drop clothing images</h2>
          <p>
            A single garment or a photo of a full outfit works. Your wardrobe stays exactly where
            you left it.
          </p>
        </div>
      </div>
      <aside
        className={`import-tray${hasImportActivity ? " is-expanded" : ""}`}
        aria-label="Wardrobe imports"
      >
        <button
          className="import-tray__button"
          type="button"
          onClick={() =>
            setupRequired || hasImportActivity ? setOpen(true) : inputRef.current?.click()
          }
          aria-label={
            setupRequired
        ...
        <div className="import-tray__actions">
          {active && (
            <img
              className="import-tray__preview"
              src={
                active.stages?.garment?.assetUrl ||
                active.stages?.garment?.failedAssetUrl ||
                active.stages?.crop?.assetUrl ||
                active.originalAssetUrl
              }
              alt=""
            />
          )}
      ...
      <div
        className="import-popover-backdrop"
        data-open={open}
                ...
                {readyCount
                  ...
                  : activeStatus?.tone === "error"
                    ...
                    : jobs.length
                      ...
                      : notice?.text || "Add to your wardrobe"}
              </h2>
            </div>
            <button
              className="import-icon-button"
          ...
          {!jobs.length ? (
            setupRequired ? (
              <div className="import-drop-target import-setup-warning">
                ...
                <p>
                  Add your OpenAI API key to <code>.env</code> and a PNG reference photo of yourself
                  at <code>{setup.modelReference || "data/model-reference.png"}</code>, then restart
                  the app.
                </p>
              </div>
            ) : (
              <div className="import-drop-target">
                ...
                <p>
                  {notice?.detail ||
                    "We’ll isolate each clothing item, suggest its details, and hold everything for your approval."}
                </p>
                <button
            ...
            <>
              <div
                className={`import-progress${activeStatus?.tone !== "processing" ? " is-reviewing" : progress < 100 ? " is-indeterminate" : ""}`}
              >
                ...
                {activeStatus?.tone === "processing" && (
                  <div className="import-progress__track">
                    <div
                      ...
                      style={{ "--import-progress": `${progress}%` }}
                    />
                  </div>
                )}
              </div>
              {reviewJob && reviewStage ? (
                ...
                <CleanupEditor
                  job={reviewJob}
                  tolerance={
                    cleanupTolerances[reviewJob.id] ??
                    reviewJob.stages.garment.cleanupTolerance ??
                    46
                  }
                  setTolerance={(tolerance) =>
                    setCleanupTolerances((current) => ({ ...current, [reviewJob.id]: tolerance }))
                  }
                  busy={busyId === reviewJob.id}
              ...
              <div className="import-card-list">
                {jobs.map((job) => {
                  ...
                  const failedStage =
                    job.stages?.garment?.status === "failed"
                      ? "garment"
                  ...
                  return (
                    <article
                      className={`import-card is-${status.tone}${reviewJob?.id === job.id ? " is-selected" : ""}`}
                      key={job.id}
                    >
                      <img
                      ...
                      <div className="import-card__body">
                        ...
                        <p
                          className="import-card__detail import-card__detail--status"
                          data-tone={status.tone}
                        >
                          {status.tone === "error" ? status.detail : status.text}
                        </p>
                      </div>
                      <div className="import-card__actions">
                        ...
                        {failedStage && (
                          <button
                            className="import-button import-card__retry"
                            disabled={busyId === job.id}
                            onClick={() => perform(job, failedStage, "regenerate", "")}
                          >
                        ...
                        <button
                          ...
                          aria-label={`Delete ${itemName} from import queue`}
                        >
                          <Trash size={16} />
                        </button>
                      </div>
