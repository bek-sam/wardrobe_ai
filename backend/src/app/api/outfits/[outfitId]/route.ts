...
async function handleDeleteOutfit(supabase: SupabaseClient, userId: string, outfitId: string) {
  const { data, error } = await supabase
    .from("outfits")
    .delete()
    .eq("id", outfitId)
...
async function handleGetOutfit(supabase: SupabaseClient, userId: string, outfitId: string) {
  const { data, error } = await supabase
    .from("outfits")
