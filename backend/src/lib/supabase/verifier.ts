...
export async function verifyPassword(email: string, password: string): Promise<boolean> {
  const environment = requireEnvironment("SUPABASE_URL", "SUPABASE_PUBLISHABLE_KEY");

  const client = createClient(environment.SUPABASE_URL, environment.SUPABASE_PUBLISHABLE_KEY, {
