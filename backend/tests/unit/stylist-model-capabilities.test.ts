...
const { requirePlannerModel, requireStylistModel } = await import("@/lib/ai/agents/orchestrator");
const { runWardrobeOrchestrator } = await import("@/lib/ai/agents/orchestrator/run");
const { hasGenerationCapability, isDeterministicOnly } = await import("@/features/stylist");
...
describe("model-backed routes fail closed", () => {
  ...
  it("still fails when the private URL is present but the workload token is not", () => {
    setEnvironment({ AI_ORCHESTRATION_URL: "http://127.0.0.1:3002" });
    expect(() => requireStylistModel()).toThrowError(
  ...
  it("returns the configured environment once the private service is authenticated", () => {
    setEnvironment({
      AI_ORCHESTRATION_URL: "http://127.0.0.1:3002",
      AI_SERVICE_TOKEN: "a".repeat(32),
    });
    expect(requireStylistModel().AI_ORCHESTRATION_URL).toBe("http://127.0.0.1:3002");
    expect(requirePlannerModel().AI_SERVICE_TOKEN).toBe("a".repeat(32));
  });
});
