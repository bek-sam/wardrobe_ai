...
describe("candidate filtering and scoring", () => {
  ...
  it("batch-scores every garment against the same complete outfit", () => {
    const outfit = [
      makeWardrobeItem({ primary_color_hex: "#1b2a4a", silhouette: "fitted" }),
      makeWardrobeItem({
        id: ITEM_IDS.bottomA,
        category: "bottoms",
        layer_role: "bottom",
        primary_color_hex: "#f4f4f0",
        silhouette: "wide leg",
      }),
      makeWardrobeItem({
        id: ITEM_IDS.shoes,
        category: "shoes",
        layer_role: "shoes",
        primary_color_hex: "#312e2b",
      }),
    ];
    const batch = scoreWardrobeOutfit(outfit);

    for (const [index, score] of batch.entries()) {
      const item = outfit[index];
      expect(item).toBeDefined();
      expect(score).toEqual(
        scoreWardrobeCandidate(item!, {
          selectedItems: outfit.filter((_candidate, candidateIndex) => candidateIndex !== index),
        }),
      );
    }
  });
