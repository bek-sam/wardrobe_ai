...
const ARCHETYPE_SCORERS: Readonly<
...
> = {
  minimalist: (items) => {
    ...
    return neutralRatio * 0.6 + (noPattern ? 0.4 : 0);
  },
  classic: (items) => {
    ...
    const neutralRatio =
      items.filter((item) => item.colorNames.every((color) => NEUTRALS.has(color.toLowerCase())))
        .length / Math.max(1, items.length);
    return (tailored / Math.max(1, items.length)) * 0.5 + neutralRatio * 0.5;
  },
  romantic: (items) =>
    items.filter((item) => /floral|flow|soft|ruffle|lace/.test(textOf(item))).length /
  ...
  bohemian: (items) =>
    items.filter((item) => /flow|earthy|paisley|fringe|layer/.test(textOf(item))).length /
    Math.max(1, items.length),
  preppy: (items) =>
    items.filter((item) => /stripe|argyle|collar|polo|blazer/.test(textOf(item))).length /
    Math.max(1, items.length),
  streetwear: (items) => {
  ...
  athleisure: (items) =>
    items.filter((item) => /athletic|jogger|legging|sneaker|performance/.test(textOf(item)))
      .length / Math.max(1, items.length),
  glam: (items) =>
    items.filter((item) => /satin|sequin|metallic|silk|velvet/.test(textOf(item))).length /
    Math.max(1, items.length),
  artsy: (items) => {
